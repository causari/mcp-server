...
interface TestCase {
  ...
  expect: (result: any) => string | null; // returns error message or null
}
...
const tests: TestCase[] = [
  {
    ...
    expect: (r) =>
      r.events?.length > 0 && r.events[0].id === 'printing'
        ? null
  ...
  },
  {
    ...
    expect: (r) =>
      r.events?.every((e: any) => e.yearNum >= 1900 && e.yearNum <= 2000 && e.impactScore >= 0.85)
        ? null
  ...
  },
  {
    ...
    expect: (r) =>
      r.events?.every((e: any) => e.domains.includes('technology') && e.impactScore >= 0.9)
        ? null
  ...
  },
  {
    ...
    expect: (r) => {
      ...
      const titles = r.effects.map((e: any) => e.id);
  ...
  },
  {
    ...
    expect: (r) => {
      ...
      const ids = r.causes.map((c: any) => c.id);
  ...
  },
  {
    ...
    expect: (r) => {
      ...
      const found = r.matches.find((m: any) => m.pattern.toLowerCase().includes('democratization'));
      return found ? null : `expected democratization pattern, got: ${r.matches.map((m: any) => m.pattern).join(', ')}`;
    },
  },
  {
    ...
    expect: (r) => {
      ...
      if (!r.scenarios.every((s: any) => typeof s.probability === 'number')) return 'scenarios missing probability';
      return null;
    },
  },
  {
  ...
  // ── AI history extension (events-ai-history.ts) ──────────────────────
  {
    ...
    expect: (r) =>
      r.events?.some((e: any) => e.id === 'chatgpt')
        ? null
        : `expected chatgpt in results, got: ${r.events?.map((e: any) => e.id).join(',')}`,
  },
  {
    tool: 'causal_chain',
...
const sample = ALL_TOOLS.find((t) => t.name === 'causal_chain')!.handler(
