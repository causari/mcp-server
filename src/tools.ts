...
export const queryEventsT: ToolDef = {
  name: 'query_events',
  description:
    'Search the Causari causal knowledge graph for historical events matching given criteria. ' +
  ...
  handler: (args, store) => {
    const result = queryEvents(store, {
      ...
      domains: args.domains as Domain[] | undefined,
      minImpact: args.minImpact as number | undefined,
...
export const causalChainT: ToolDef = {
  name: 'causal_chain',
  description:
    'Trace cause-effect chains from a historical event. Returns structured causal graph showing what led to the event (causes) and/or what it enabled (effects), with evidence and confidence scores. ' +
  ...
  handler: (args, store) => {
    const result = causalChain(store, {
      ...
      direction: args.direction as CausalDirection | undefined,
    ...
    const r = result as Exclude<typeof result, { notFound: string }>;
...
export const historicalResonanceT: ToolDef = {
  name: 'historical_resonance',
  description:
    'Find historical patterns similar to a current situation. Useful for understanding contemporary events through historical parallels — e.g., "rapid AI adoption displacing knowledge workers" might resonate with the printing press disrupting clergy. ' +
...
export const orgKnowledgeT: ToolDef = {
  name: 'org_knowledge',
  description:
    "[Enterprise tier] Query the organization's internal causal knowledge graph — decisions, events, technical milestones, and their consequences. " +
...
export const predictScenariosT: ToolDef = {
  name: 'predict_scenarios',
  description:
    'Generate plausible future scenarios based on historical causal patterns and current conditions. ' +
  ...
  handler: (args, store) => {
    ...
    return {
      ...
      meta: {
        ...result.basedOn,
        disclaimer:
          'Scenarios are derived from historical pattern matching, not guaranteed forecasts. ' +
